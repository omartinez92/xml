...
foreach ($files as $file) {
	...
	if (substr($file, strrpos($file, '.')+1) == 'xml') {
        ...
        $xml = simplexml_load_file(__DIR__."/notebook/$file");
        ...
        $info = array(
			...
			'osinstalledftntnbr' => (string) osinstalledftntnbr,
			'processorfamily' => (string) $xml->content->tech_specs->processor->processorfamily,
			'hd_01des' =>  $xml->content->tech_specs->drives->hd_01des,
			'hd_01desftntnbr' => $xml->content->tech_specs->drives->hd_01desftntnbr,
			'graphicseg_01card_01' => $xml->content->tech_specs->graphic_subsystem->graphicseg_01card_01,
			'graphiccardftntnbr' => $xml->content->tech_specs->graphic_subsystem->graphiccardftntnbr,
			'ioports' => $xml->content->tech_specs->connectivity_and_communications->ioports,
			'wirelesstech' => (string) $xml->content->tech_specs->connectivity_and_communications->wirelesstech,
			'wirelesstechftntnbr' => (string) wirelesstechftntnbr,
			'batterylifedes' => (string) $xml->content->tech_specs->battery_and_power->batterylifedes,
			'batterylifedesftntnbr' => (string) batterylifedesftntnbr,
